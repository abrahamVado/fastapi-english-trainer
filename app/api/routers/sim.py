...
from app.schemas.sim import (
    ...
    SimAnswerTextReq, SimAnswerAudioRes, SimScoreReq, SimScoreRes, ScoreBreakdown, SimReportRes
)
...
def _ask_first_question(role: str, level: str, mode: str) -> str:
    # placeholder; later route to LLM interviewer
...
async def sim_answer_audio(
    ...
    # Tip: add an initial_prompt later with role keywords to bias ASR
    asr_text = await _asr.transcribe(data, language="en")
    t["answer_text"] = asr_text or ""
    return SimAnswerAudioRes(session_id=session_id, question_id=question_id, asr_text=asr_text or "", confidence=None)

@router.post("/score", response_model=SimScoreRes)
def sim_score(req: SimScoreReq):
    turns = _TURNS.get(req.session_id)
    if not turns:
        raise HTTPException(404, "session not found")
    t = next((t for t in turns if t["qid"] == req.question_id), None)
    ...
    judge = _judge.judge_content(_CONTENT_RUBRIC, evidence)
    try:
        content = int(judge.get("score", 0))
    except Exception:
        content = 0
    ...
    if req.expected_text:
        ...
        pronunciation = int(pron_json["overall"]["score_0_100"])
    else:
    ...
    tips = judge.get("tips") or ["Add a concrete example.", "Explain trade-offs and impact.", "Keep answers structured (STAR)."]

    return SimScoreRes(
        scores=ScoreBreakdown(content=content, pronunciation=pronunciation, fluency=fluency, overall=overall),
        tips=tips
    )
